...
```mermaid
graph TD
    A[Google AI Studio'ya Git] --> B[Google Hesabı ile Giriş]
    B --> C[Create API Key'e Tıkla]
    C --> D[API Anahtarını Kopyala]
    D --> E[Extension'a Yapıştır]
    E --> F[Kaydet ve Test Et]
...
```mermaid
graph LR
    A[Repository İndir] --> B[Chrome Extensions Aç]
    B --> C[Developer Mode Aktif Et]
    C --> D[Load Unpacked Tıkla]
    D --> E[Klasör Seç]
    E --> F[Extension Yüklendi!]
...
sequenceDiagram
    ...
    U->>E: Metin seç ve sağ tık
    E->>E: "AI ile Analiz Et" menüsü
    U->>E: Menüye tıkla
    E->>A: Metni gönder
    A->>E: İşlenmiş metin
    E->>U: Popup'ta sonucu göster
```
...
```mermaid
graph TD
    A[Popup'ta Ayarlar] --> B[Tool Amacı Seç]
    B --> C{Çeviri mi?}
    C -->|Evet| D[Hedef Dil Seç]
    C -->|Hayır| E[Kaydet]
    D --> E
    E --> F[Chat'e Dön]
    F --> G[Hazır!]
